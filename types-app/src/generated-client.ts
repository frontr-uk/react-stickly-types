...
import { z } from "zod";
import { getApiUrl, remapApiClient } from "../../types-api/sdk/utils";
import qs from "qs";
...
const petZodios = (baseUrl: string, options?: ZodiosOptions) =>
  new Zodios(baseUrl, petEndpoints, options);

export async function createPetClient(
  options: ZodiosOptions = { validate: "request" }
) {
  const apiUrl = getApiUrl();
  const client = petZodios(apiUrl, {
    axiosConfig: {
      paramsSerializer: {
...
const storeZodios = (baseUrl: string, options?: ZodiosOptions) =>
  new Zodios(baseUrl, storeEndpoints, options);

export async function createStoreClient(
  options: ZodiosOptions = { validate: "request" }
) {
  const apiUrl = getApiUrl();
  const client = storeZodios(apiUrl, {
    axiosConfig: {
      paramsSerializer: {
        serialize: (params) => qs.stringify(params, { arrayFormat: "repeat" }),
      },
...
const userZodios = (baseUrl: string, options?: ZodiosOptions) =>
  new Zodios(baseUrl, userEndpoints, options);

export async function createUserClient(
  options: ZodiosOptions = { validate: "request" }
) {
  const apiUrl = getApiUrl();
  const client = userZodios(apiUrl, {
    axiosConfig: {
      paramsSerializer: {
        serialize: (params) => qs.stringify(params, { arrayFormat: "repeat" }),
      },
