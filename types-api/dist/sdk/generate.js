...
function makeAlias(path, method, operation) {
    ...
    return operation.operationId
        ? lcFirst(operation.operationId.split("_").map(server_1.capitalize).join(""))
        : `${method}${pathToVariableName(basePath)}`;
}
const pathParamWithBracketsRegex = /({\w+})/g;
const pathToVariableName = (path) => path
    .split("/")
    .map((part) => (0, server_1.capitalize)((0, server_1.kebabToCamel)(part
